...
--    │  ● arbitration_log   │   Veritas Proxy audit trail
--    └──────────┬───────────┘
--               │
--    ┌──────────▼───────────┐
...
CREATE TABLE IF NOT EXISTS shard_map (
    ...
    -- The VeritasProxy MUST call SecurityManager.decryptField() before using this value.
...
-- The Veritas Proxy queries this on EVERY /v1/arbitrate call.
...
-- ─────────────────────────────────────────────────────
--  VERITAS PROXY AUDIT LOG — Arbitration Trail
-- ─────────────────────────────────────────────────────

-- Every call to /v1/arbitrate is logged here.
-- This is the governance audit trail for the Veritas Proxy.
...
CREATE INDEX IF NOT EXISTS idx_veritas_time 
    ON veritas_audit_log (created_at DESC);

-- Decision analytics
CREATE INDEX IF NOT EXISTS idx_veritas_decision 
    ON veritas_audit_log (arbiter_decision, created_at DESC);

-- Partition hint: For production volumes, partition by month:
-- CREATE TABLE veritas_audit_log (...) PARTITION BY RANGE (created_at);
...
-- Uses current_setting('sentinel.tenant_id') set by the Veritas Proxy
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'freight_indices') THEN
