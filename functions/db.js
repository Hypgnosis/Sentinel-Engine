...
function getSql() {
  ...
  _sql = postgres(url, {
    ssl: 'require',
    max: 50,              // Tier 1 traffic — 10 was free-tier budget
    idle_timeout: 30,
    connect_timeout: 5,   // 5s acquisition from pool before rejecting
    max_lifetime: 1800,   // Recycle connections every 30min to prevent stale handles
  });
  return _sql;
}
