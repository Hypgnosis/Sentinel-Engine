...
class SecurityManager {
  ...
  static create(providerType = 'software', options = {}) {
    switch (providerType) {
      case 'software': {
        // Use provided keys or generate deterministic defaults from env
        const encKey = options.encryptionKey
          || process.env.SENTINEL_ENCRYPTION_KEY
          || crypto.createHash('sha256').update(process.env.DATABASE_URL || 'sentinel-default-key-v49').digest('hex');

        const sigKey = options.signingKey
          || process.env.SENTINEL_SIGNING_KEY
          || crypto.createHash('sha256').update((process.env.DATABASE_URL || 'sentinel-signing-v49') + '-sign').digest('hex');
        ...
        console.log('[SECURITY_MANAGER] Initialized with SoftwareKmsProvider.');
